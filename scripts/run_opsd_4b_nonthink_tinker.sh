#!/usr/bin/env bash
# Run OPSD on Qwen3-4B (non-thinking mode) with the Tinker-style sampled-token
# loss for any of the 5 supported f-divergences.
#
# Usage:
#   bash scripts/run_opsd_4b_nonthink_tinker.sh <divergence_type> [--smoke]
#
#   divergence_type: reverse_kl | forward_kl | jsd | improved_forward_kl | improved_jsd
#   --smoke: short run for pipeline validation
#
# Env overrides:
#   MODEL  (default: /data0/shared/Qwen3-4B)  -- HF id or local path
...
MODEL="${MODEL:-/data0/shared/Qwen3-4B}"
OUT="${OUT:-/data0/siyanz/opsd}"

if [[ "$SMOKE" == "--smoke" ]]; then
    RUN_CONFIG="smoke_4b_nonthink_${DIV}"
    ...
    RUN_CONFIG="qwen34b_nonthink_tinker_${DIV}"
    STEP_ARGS=(--num_train_epochs 30 --save_steps 25 --logging_steps 2 --max_completion_length 1024)
fi

echo "[run_opsd_4b_nonthink_tinker] MODEL=$MODEL"
echo "[run_opsd_4b_nonthink_tinker] OUT=$OUT"
echo "[run_opsd_4b_nonthink_tinker] divergence_type=$DIV  run_config=$RUN_CONFIG"
