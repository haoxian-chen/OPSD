...
if [[ "$SMOKE" == "--smoke" ]]; then
    ...
    RUN_CONFIG="qwen34b_tinker_${DIV}"
    STEP_ARGS=(--num_train_epochs 30 --save_steps 25 --logging_steps 2 --max_completion_length 1024)
fi

echo "[run_opsd_4b_tinker] MODEL=$MODEL"
echo "[run_opsd_4b_tinker] OUT=$OUT"
echo "[run_opsd_4b_tinker] divergence_type=$DIV  run_config=$RUN_CONFIG"
