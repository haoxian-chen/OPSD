#!/usr/bin/env bash
# Run OPSD on Qwen3-8B (non-thinking mode) with the Tinker-style sampled-token
# loss for any of the 6 supported f-divergences.
#
# Usage:
#   bash scripts/run_opsd_8b_nonthink_tinker.sh <divergence_type> [--smoke]
#
...
if [[ "$SMOKE" == "--smoke" ]]; then
    RUN_CONFIG="smoke_8b_nonthink_${DIV}"
    ...
    RUN_CONFIG="qwen38b_nonthink_tinker_${DIV}"
    STEP_ARGS=(--num_train_epochs 30 --save_steps 25 --logging_steps 2 --max_completion_length 1024)
fi

echo "[run_opsd_8b_nonthink_tinker] MODEL=$MODEL"
echo "[run_opsd_8b_nonthink_tinker] OUT=$OUT"
echo "[run_opsd_8b_nonthink_tinker] divergence_type=$DIV  run_config=$RUN_CONFIG"
