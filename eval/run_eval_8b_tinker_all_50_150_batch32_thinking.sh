...
# thinking mode enabled. This variant evaluates checkpoints 50, 100, and 150
# on aime24, aime25, and hmmt25 with VAL_N=32 by default.
#
# Usage:
#   bash eval/run_eval_8b_tinker_all_50_150_batch32_thinking.sh
#
...
#   SKIP_EXISTING=1  # skip evals, including completed aime24 evals, whose result JSON already exists
...
STEP_LIST="${STEP_LIST:-50,100,150}"
MAX_ALLOWED_STEP="${MAX_ALLOWED_STEP:-150}"
...
WANDB_TAGS="${WANDB_TAGS:-eval,8b,tinker,thinking,all,valn32}"
...
for dataset in "${DATASET_LIST[@]}"; do
    wandb_group="${WANDB_GROUP_OVERRIDE:-eval_qwen38b_tinker_all_${dataset}_thinking_valn${VAL_N}}"
    EVAL_ARGS=(
    ...
    for step in "${STEPS[@]}"; do
        if [[ "$step" == "0" ]]; then
            if [[ "$BASE_DONE" == "0" ]]; then
                ...
                NCCL_P2P_DISABLE=1 CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" python evaluate_math.py \
                    ...
                    --wandb_run_name "eval_qwen38b_tinker_base_${dataset}_thinking_valn${VAL_N}"
                BASE_DONE=1
            fi
            continue
        fi
        ...
        for div in "${DIVERGENCE_LIST[@]}"; do
            ...
            NCCL_P2P_DISABLE=1 CUDA_VISIBLE_DEVICES="$CUDA_VISIBLE_DEVICES" python evaluate_math.py \
                ...
                --wandb_run_name "eval_${run_config}_checkpoint-${step}_${dataset}_thinking_valn${VAL_N}"
        done
    done
done
